...
class FragNet(nn.Module):
    ...
    def __init__(self, num_layer, drop_ratio=0, emb_dim=128,
        ...
        self.layer1 = FragNetLayer(atom_in=atom_features, atom_out=emb_dim,
                                   frag_in=frag_features, frag_out=emb_dim,
                                   edge_in=edge_features, edge_out=emb_dim)
        self.layer2 = FragNetLayer(atom_in=emb_dim, atom_out=emb_dim,
                                   frag_in=emb_dim, frag_out=emb_dim,
                                   edge_in=edge_features, edge_out=emb_dim)
        self.layer3 = FragNetLayer(atom_in=emb_dim, atom_out=emb_dim,
                                   frag_in=emb_dim, frag_out=emb_dim,
                                   edge_in=edge_features, edge_out=emb_dim)
        self.layer4 = FragNetLayer(atom_in=emb_dim, atom_out=emb_dim,
                                   frag_in=emb_dim, frag_out=emb_dim,
                                   edge_in=edge_features, edge_out=emb_dim)
...
class FragNetPreTrain(nn.Module):
    ...
    def __init__(self, atom_features=167, frag_features=167, edge_features=17,
                emb_dim=128, num_layer=6, drop_ratio=0.15):
        super(FragNetPreTrain, self).__init__()

        self.pretrain = FragNet(
            num_layer=num_layer,       # ✅ 不再硬编码
            drop_ratio=drop_ratio,     # ✅ 不再硬编码
            emb_dim=emb_dim,           # ✅ 不再硬编码
...
class FragNetFineTune(nn.Module):
    ...
    def __init__(self, n_classes=1,
                 atom_features=167, frag_features=167, edge_features=17,  # ✅ 新增
                 emb_dim=128, drop_ratio=0.15, num_layer=6):               # ✅ 新增
        ...
        self.pretrain = FragNet(
            ...
            emb_dim=emb_dim,
            atom_features=atom_features,   # ✅ 透传
            frag_features=frag_features,   # ✅ 透传
            edge_features=edge_features,   # ✅ 透传
        )
